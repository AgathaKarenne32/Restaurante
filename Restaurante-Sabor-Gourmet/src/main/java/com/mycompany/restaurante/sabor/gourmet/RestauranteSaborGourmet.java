...
public class RestauranteSaborGourmet {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            System.out.println("5. Verificar status do pedido");
    ...
    private static void finalizarPedido(Scanner scanner) {
        ...
        if (numPedidos > 0) {
            ...
            if (opcaoPedido >= 1 && opcaoPedido <= numPedidos) {
                ...
                // Atualiza o status para "Pronto para entrega" se ainda não estiver finalizado
            if (!pedidoParaFinalizar.getStatus().equalsIgnoreCase("Entregue")) {
                pedidoParaFinalizar.atualizarStatus("Pronto para entrega");
                System.out.println("Pedido está pronto para entrega.");
            }
                pedidoParaFinalizar.atualizarStatus("Entregue");
    ...
    private static void verificarStatusPedido(Scanner scanner) {
        System.out.println("\n=== VERIFICAR STATUS DO PEDIDO ===");
        if (numPedidos > 0) {
            System.out.println("Escolha o número do pedido para verificar o status:");
            for (int i = 0; i < numPedidos; i++) {
                System.out.println((i + 1) + ". Pedido " + (i + 1));
            }
            int opcaoPedido = scanner.nextInt();
            scanner.nextLine();  // Consume newline
            if (opcaoPedido >= 1 && opcaoPedido <= numPedidos) {
                Pedido pedidoParaVerificar = pedidos[opcaoPedido - 1];
                System.out.println("Status do pedido:");
                System.out.println("Pedido " + opcaoPedido + ": " + pedidoParaVerificar.getStatus());
            } else {
                System.out.println("Número do pedido inválido.");
            }
        } else {
            System.out.println("Nenhum pedido para verificar.");
        }
    }
}
